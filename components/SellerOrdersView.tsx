    ...
    <div className="max-w-4xl mx-auto py-6 space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-stone-900 dark:text-stone-100">
          {t.orders_title} (Panel Artesano)
        </h1>
        <p className="text-xs text-stone-500 dark:text-stone-400">
          Valida y actualiza los pedidos de tus clientes en tiempo real.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {orders.length > 0 ? (
          orders.map((order) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-stone-100 dark:border-stone-800">
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      ...
                      title={t.orders_chat_with_buyer}
                    >
                  ...
                  <div className="p-3 rounded-2xl bg-stone-50 dark:bg-stone-850 border border-stone-200 dark:border-stone-800 flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      {order.shipping_notes && (
                        <p className="text-stone-400 text-[10px] italic mt-0.5">
                ...
                {/* 4. Importe Total y Selector de Estados (Fila 1) + Cancelar (Fila 2) */}
                <div className="pt-3 border-t-2 border-stone-100 dark:border-stone-800 space-y-2.5">
                  <div className="flex items-end justify-between gap-3">
                    {/* Selector de Estado a la Izquierda */}
                    <div className="flex flex-col gap-1 min-w-0 flex-1 max-w-xs">
                      <label className="text-[11px] font-black text-stone-600 dark:text-stone-400 block">
                        {t.orders_status}:
                      </label>
                      <select
                        value={order.status}
                        ...
                        className="w-full text-xs font-black px-3 py-2 rounded-xl border border-stone-300 dark:border-stone-700 bg-white dark:bg-stone-800 text-stone-900 dark:text-stone-100 focus:outline-none focus:ring-2 focus:ring-amber-500 cursor-pointer shadow-2xs"
                      >
                        <option value="pendiente">{t.orders_pending}</option>
                        <option value="confirmado">{t.orders_confirmed}</option>
                        <option value="preparando">{t.orders_preparing}</option>
                        <option value="listo_entrega">{t.orders_ready_delivery}</option>
                        <option value="entregado">{t.orders_delivered}</option>
                  ...
                  {/* Fila 2: Cancelar Pedido a Ancho Completo */}
          ...
          <div className="py-16 text-center bg-white dark:bg-stone-900 rounded-3xl border-2 border-stone-200 dark:border-stone-800 p-8 space-y-3">
            ...
            <p className="text-xs text-stone-500 dark:text-stone-400">
              Aquí aparecerán los pedidos que realicen tus clientes.
            </p>
          </div>
        )}
      </div>
