...
// 1. components/Navbar.tsx (Pasa únicamente los pedidos pendientes a los vendedores)
saveFile('components/Navbar.tsx', `
...
export default async function Navbar() {
  ...
  if (user) {
    ...
    const isSellerOrAdmin = profile?.role === 'vendedor' || profile?.role === 'admin';

    const [unreadRes, ordersRes] = await Promise.all([
      supabase
        .from('chat_messages')
      ...
      isSellerOrAdmin
        ? supabase
            .from('orders')
            .select('id, status')
            .eq('status', 'pendiente') // Vendedores: solo pedidos pendientes de validar
...
// 2. components/NavbarNavLinks.tsx (Verificación estricta: 0 iluminaciones si no hay pedidos pendientes sin ver)
...
export function NavbarNavLinks({
  ...
  useEffect(() => {
    function checkUnseenOrders() {
      ...
      if (isSeller) {
        // Filtra estrictamente solo los que están en estado "pendiente"
        ...
        // Solo se ilumina si existe al menos un pedido pendiente que NO esté marcado como visto
        const hasUnseenPending = pendingOrders.some((order) => seenMap[order.id] !== 'pendiente');
        setHasUnseenOrderUpdates(hasUnseenPending);
      } else {
        // Compradores
        let seenMap: Record<string, string> = {};
        try {
...
console.log('\n✨ Verificación estricta de iluminación de pedidos para vendedores completada.');